...
:is(html[data-theme="midnight"]) .tg-break{background:repeating-linear-gradient(135deg,#2b2110 0 5px,#33270f 5px 10px)}
:is(html[data-theme="midnight"]) .tg-break span{color:var(--amber-deep)}
:is(html[data-theme="forest"]) .tg-break{background:repeating-linear-gradient(135deg,#2c2410 0 5px,#35290c 5px 10px)}
:is(html[data-theme="forest"]) .tg-break span{color:var(--amber-deep)}
...
function renderAllocationPops(){
  ...
  for(const pop of ALLOC_POPS){
    ...
    h += '<div class="sec-add-row">'
      + '<input id="secKey-' + pop + '" placeholder="Section id — e.g. ' + (pop === 'bs-1' ? 'BSAF-SEM-II' : 'ICS-II-A') + '" maxlength="24">'
      + '<select id="secProg-' + pop + '">' + (pop === 'bs-1'
          ? '<option value="BSAF">BSAF</option><option value="BSCM">BSCM</option><option value="BBA">BBA</option>'
          : '<option value="ICS">ICS</option><option value="I.COM">I.Com</option>') + '</select>'
      + '<select id="secYear-' + pop + '">' + (pop === 'bs-1'
          ? ['I','II','III','IV','V','VI','VII','VIII'].map(function(s){ return '<option value="' + s + '">Semester ' + s + '</option>'; }).join('')
          : '<option value="I">Year I</option><option value="II">Year II</option>') + '</select>'
