...
| 📋 **Instructions** | Institution-level general instructions, styled exactly like the admin's Excel rulebook: **one linear sheet per shift** (Shift 1 = Intermediate + BS together; Shift 2 = Intermediate 2nd). Each row is **topic → plain-language detail → editable pseudocode expression** — translations are pattern-first and AI-backed, and every row applies / deletes atomically into the solver's own data channels. Draft → ☁ Publish per shift; edits govern future generations (JS and CP-SAT alike). |
